...
- Earlier parts of the conversation may arrive as documents of their own, before the main one, each holding an \`earlierMessages\` list. They are the same conversation and read in the same way — they come separately only so the bot does not pay to re-read them every time. Treat them as sitting immediately before \`messages\`.
