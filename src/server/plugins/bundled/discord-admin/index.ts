...
const plugin: BigYahuPlugin = {
  ...
  instructions(ctx) {
    ...
    return `Discord administration is available only while replying to a configured Big Yahu controller. The host, not you, verifies that identity and withholds every admin tool on other turns.
...
- Mutations may return an exact, payload-bound CONFIRM phrase. Never write or call that confirmation yourself. Ask the controller to send it in a new Discord message and stop until they do. Irreversible actions and Administrator grants always require this confirmation.
